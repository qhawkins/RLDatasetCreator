...
int random_ideal_selection() {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> distrib(0, 9);

    int randomIdeal = distrib(gen);
    //std::cout << "random ideal: " << randomIdeal << std::endl;
    return randomIdeal;
}
...
int main() {
    ...
    std::string price_str = "/mnt/drive2/shared_data/merged_data/numpy/merged_arrays/mid_prices.csv";
    std::string length_list_str = "/mnt/drive2/shared_data/merged_data/numpy/merged_arrays/length_list.csv";
    ...
    for (int i = 0; i < length_list.size(); ++i){
        ...
        for (int timestep = length_list[i]-price_length; timestep < length_list[i]-end_of_ep_buffer; ++timestep){
            ...
            int random_selection = random_ideal_selection();
